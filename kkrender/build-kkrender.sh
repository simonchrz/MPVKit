...
build_slice() {  # build_slice <sdk> <min-flag> <plinc> <spvcdir> <outdir>
  ...
  for src in hybrid_render kk_renderer kk_gpu_render kk_gpu_cnn; do
    "$CLANG" -c "$ROOT/$src.c" -o "$od/$src.o" \
      -arch arm64 -isysroot "$sysroot" "$min" ${KK_AOT:+-DKK_AOT} \
      -I"$plinc" -I"$ROOT" -Wall -O2 -fno-common
  done
  # kk_gpu.m = eigene Metal-Foundation (ObjC, ARC). kk_gpu_render.c ist der gated
  # libplacebo-freie SDR-Pfad (KUCKUCK_KK_GPU=1, Strangler in hybrid_render.c).
  ...
  if [ -n "${KK_AOT:-}" ]; then
    "$LIBTOOL" -static -o "$od/libkkrender.a" \
      "$od/hybrid_render.o" "$od/kk_renderer.o" "$od/kk_gpu_render.o" "$od/kk_gpu_cnn.o" "$od/kk_gpu.o"
  else
    "$LIBTOOL" -static -o "$od/libkkrender.a" \
      "$od/hybrid_render.o" "$od/kk_renderer.o" "$od/kk_gpu_render.o" "$od/kk_gpu_cnn.o" "$od/kk_gpu.o" "$spvc"/*.a
  fi
