...
build_slice() {  # build_slice <sdk> <min-flag> <plinc> <spvcdir> <outdir>
  ...
  for src in hybrid_render kk_renderer kk_gpu_render; do
  ...
  if [ -n "${KK_AOT:-}" ]; then
    "$LIBTOOL" -static -o "$od/libkkrender.a" \
      "$od/hybrid_render.o" "$od/kk_renderer.o" "$od/kk_gpu_render.o" "$od/kk_gpu.o"
  else
    "$LIBTOOL" -static -o "$od/libkkrender.a" \
      "$od/hybrid_render.o" "$od/kk_renderer.o" "$od/kk_gpu_render.o" "$od/kk_gpu.o" "$spvc"/*.a
  fi
